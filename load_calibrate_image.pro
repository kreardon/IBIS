...
;     no_interpolation - do not apply any corrections that require pixel interpolation (fractional rotation, remapping, etc.)
...
;     cal_info_out  - calibration information in structure from load_calibration_info
;     gain_out      - gain table applied to the data 
;     shifts_out    - the [x,y] shifts applied to the data
; RESULTS:
;
; EXAMPLE:
;	  
...
; ----------------------------------------------------------------------
; read in desired image and extract metadata 
; ----------------------------------------------------------------------
...
IF NOT Keyword_Set(no_interpolation) THEN BEGIN
    IF Keyword_Set(filter_fringe) AND (channel_id EQ 'nb') THEN BEGIN
        ...
        image_array = filter_fringe_fft(image_array,fe5434=filter_idx[0],na5896=filter_idx[1],$
        					    halpha=filter_idx[2],fe7090=filter_idx[3],$
        					    k7699=filter_idx[4],ca8542=filter_idx[5])
    ENDIF
ENDIF ; no_interpolation
   
; ----------------------------------------------------------------------
; rotate to nominal orientation (solar north up); correct image rotation
; ----------------------------------------------------------------------
...
; ----------------------------------------------------------------------
IF have_distortion_map AND Keyword_Set(apply_distortion_map) AND NOT Keyword_Set(no_interpolation) THEN BEGIN
    device_orig = !D
    set_plot,'Z'
...
; ----------------------------------------------------------------------
; apply bulk shifts to align all channels to common reference 
; ----------------------------------------------------------------------
...
IF Keyword_Set(no_interpolation) THEN image_shift = ROUND(image_shift)
